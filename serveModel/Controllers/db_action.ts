...
import { dbInfo, serverOption, httpOptions } from '../../config/config.base'
...
export default class dbAction {
    ...
    */
    async updateAction(queryBody: any) {
        let dbInfo: dbInfo = new Config('mysqlConfigDev', '').config;
    ...
    */
    async getTaskStation(queryBody: any) {
        let dbListInfo: Array<dbInfo> = new Config('', '').dbListInfo;
        let HostListInfo: Array < serverOption >= new Config('', '').HostListInfo;
    ...
    **/
    asyncgetEmployeeInfo(ctx: Context) {
        ...
        let hostConfig: serverOption = new Config('', '').rjck_prd;
